...
def equations(nlist, hlist): #packaged phenotypes and hA, hB
    """
...
def gene_counting(): #run gene counting algorithm, with loop.
    # initialize parameters
    nA = 44; nB = 27; nAB = 4; nO = 88 #sample given from Morton (1964)
    hA = 0.5; hB = 0.5  # initial guesses for hA and hB

    rel_criteria = 1e-4   # loop termination criteria for 

    # pack parameters into list
    nlist = [nA, nB, nAB, nO]
    hlist = [hA, hB]

    # initialize paramters and log likelihood
    theta, hlist = equations(nlist, hlist)
    ...
    while True:
        ...
        rel_diff = []   # list to store relative difference
    ...
    # plot p vs q 
    # (can refactor if needed DELETE THIS COMMENT ANABEL)
    ...
    plt.savefig(f'plots/p_vs_q.png')

if __name__=="__gene_counting__":
    gene_counting()
